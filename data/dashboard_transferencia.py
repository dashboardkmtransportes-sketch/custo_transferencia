...
with tab1:
    if df_filtrado.empty:
        ...
        if not df_viagens.empty:
            ...
            ### MUDANÇA 1: FUNÇÃO PARA CALCULAR A DISTÂNCIA POR VIAGEM ###
            ...
            ### MUDANÇA 2: APLICA A FUNÇÃO DE DISTÂNCIA ###
            ...
            resumo_viagens['CTRB/Frete (%)'] = resumo_viagens['CTRB/Frete (%)_valor'].apply(lambda x: f"{x:.0f}%".replace(".", ","))
            ...
            resumo_viagens['CAPAC_CAVALO'] = resumo_viagens['CAPAC_CAVALO'].astype(float).apply(lambda x: formatar_numero(x, 0) + ' kg')
            
            ### MUDANÇA 3: FORMATA A COLUNA DE DISTÂNCIA ###
            ...
            ### MUDANÇA 4: ADICIONA A COLUNA 'DISTANCIA' NA ORDEM FINAL ###
            ordem_final = [
                'VIAGEM', 'EMISSÃO', 'Nº Manifesto', 'SITUAÇÃO', 'MOTORISTA', 
                'CTRB/Frete (%)',
                'DESTINOS',
                'DISTANCIA', # <-- COLUNA ADICIONADA AQUI
                'ENTREGAS', 'TIPO', 
...
with tab5:
    ...
    locais = {
        ...
        "AGUA CLARA/MS": [-20.4486, -52.8783], "ACL": [-20.4486, -52.8783],
        "ANASTACIO/MS": [-20.4953, -55.8089],
        "APARECIDA DO TABOADO/MS": [-20.0867, -51.0933], "ATB": [-20.0867, -51.0933],
        "BATAGUASSU/MS": [-21.715, -52.4225],
        "BODOQUENA/MS": [-20.5507, -56.67928], "BDQ": [-20.5507, -56.67928],
        "BONITO/MS": [-21.1236, -56.4934],
        "CAMAPUA/MS": [-19.5349, -54.0432], "CMP": [-19.5349, -54.0432],
        "CASSILANDIA/MS": [-19.1133, -51.7339], "CSL": [-19.1133, -51.7339],
        "CHAPADAO DO SUL/MS": [-18.7955, -52.6012], "CDS": [-18.7955, -52.6012],
        "CORUMBA/MS": [-19.0205, -57.6578],
        "COSTA RICA/MS": [-18.5432, -53.1292], "CRC": [-18.5432, -53.1292],
        "COXIM/MS": [-18.5134, -54.7406], "COX": [-18.5134, -54.7406],
        "DOURADOS/MS": [-22.2204, -54.7360], "DOU": [-22.2204, -54.7360],
        "GOIANIA/GO": [-16.6869, -49.2648], "GYN": [-16.6869, -49.2648],
        "IVINHEMA/MS": [-22.3067, -53.8153], "IVM": [-22.3067, -53.8153],
        "JARDIM/MS": [-21.4803, -56.1381], "JDM": [-21.4803, -56.1381],
        "MARACAJU/MS": [-21.6147, -55.1683], "MJU": [-21.6147, -55.1683],
        "MIRANDA/MS": [-20.2363, -56.3812], "MDA": [-20.2363, -56.3812],
        "NIOAQUE/MS": [-21.1576, -55.8446], "NQU": [-21.1576, -55.8446],
        "NOVA ALVORADA DO SUL/MS": [-21.4631, -54.3844], "NSU": [-21.4631, -54.3844],
        "NOVA ANDRADINA/MS": [-22.235, -53.3433],
        "PARANAIBA/MS": [-19.6834, -51.1968], "PBA": [-19.6834, -51.1968],
        "PARAISO DAS AGUAS/MS": [-19.0202, -53.0094], "PDA": [-19.0202, -53.0094],
        "PEDRO GOMES/MS": [-18.1011, -54.5525], "PGO": [-18.1011, -54.5525],
        "PONTA PORA/MS": [-22.5609, -55.6945],
        "RIBAS DO RIO PARDO/MS": [-20.4433, -53.7592], "RRP": [-20.4433, -53.7592],
        "RIO BRILHANTE/MS": [-21.8036, -54.5456], "RBT": [-21.8036, -54.5456],
        "RIO VERDE DE MATO GROSSO/MS": [-18.9181, -54.8453], "RVM": [-18.9181, -54.8453],
        "SAO GABRIEL DO OESTE/MS": [-19.395, -54.56], "SGO": [-19.395, -54.56],
        "SAO PAULO/SP": [-23.5183, -46.5729], "SPO": [-23.5183, -46.5729],
        "SIDROLANDIA/MS": [-20.9319, -54.9603], "SDL": [-20.9319, -54.9603],
        "SONORA/MS": [-17.5736, -54.7558], "SNR": [-17.5736, -54.7558],
        "TERENOS/MS": [-20.4428, -55.7597],
        "TRES LAGOAS/MS": [-20.7519, -51.6783], "TLG": [-20.7519, -51.6783],
    }
    ...
    if st.session_state.get('rota_clicada_id') is None:
        ...
        m = folium.Map(
            location=[-19.5, -54.5],
            zoom_start=7,
            tiles='https://mt1.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}', # <-- SATÉLITE AGORA É O PADRÃO
            attr='Google',
            name='Google Maps - Satélite'
        )
        folium.TileLayer(
            'https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', # <-- RUAS AGORA É A OPÇÃO
            attr='Google',
        ...
        if df_filtrado is None or df_filtrado.empty or 'PLACA_CAVALO' not in df_filtrado.columns:
            ...
            map_data = st_folium(m, use_container_width=True, height=700, key="mapa_geral")
        ...
        ### NOVO FILTRO DE EXCEÇÃO: REMOVER MANIFESTO ERRADO DA ROTA ###
        # Esta regra se aplica apenas à viagem específica com a placa 'RLN5H67'
        if placa_selecionada == 'RLN5H67':
            # Define o número do manifesto que foi incluído por engano
            manifesto_para_remover = 'CGR102822-7'
            
            # Verifica se a coluna 'NUM_MANIF' existe e se o manifesto está presente
            if 'NUM_MANIF' in df_detalhe.columns and manifesto_para_remover in df_detalhe['NUM_MANIF'].values:
                
                # Remove a linha correspondente ao manifesto errado
                df_detalhe = df_detalhe[df_detalhe['NUM_MANIF'] != manifesto_para_remover]
                
                # (Opcional) Adiciona um aviso na interface para informar sobre a correção automática
                st.info(f"ℹ️ **Correção Automática:** O manifesto `{manifesto_para_remover}` (destino SDL) foi removido desta rota por ser um erro de lançamento.")
        ### FIM DO FILTRO DE EXCEÇÃO ###
        ...
        if not df_detalhe.empty:
            ...
            def obter_cor_ociosidade(percentual):
                # Retorna sempre o gradiente amarelo, ignorando o valor do percentual.
        ...
        mapa_detalhe = folium.Map(
            location=[-19.5, -54.5],
            zoom_start=5,
            tiles='https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', # <-- RUAS (AGORA É O PADRÃO )
            attr='Google',
            name='Google Maps - Ruas'
        )
        folium.TileLayer(
            'https://mt1.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}', # <-- SATÉLITE (AGORA É A OPÇÃO )
            attr='Google',
            name='Google Maps - Satélite'
        ).add_to(mapa_detalhe)
        ...
        if not df_detalhe.empty:
            ...
            if origem_nome in locais:
                ...
                folium.Marker(locais[origem_nome], 
                            popup=f"Origem: {origem_nome}",
            ...
            destinos_siglas_da_viagem = set(df_detalhe['DEST_MANIF'].unique())
            rota_sao_paulo_destinos = {'CSL', 'PBA', 'ATB', 'SPO'}

            if rota_sao_paulo_destinos.issubset(destinos_siglas_da_viagem):
                destinos_ordenados = ['CSL', 'PBA', 'ATB', 'SPO']
                destinos_ordenados = [dest for dest in destinos_ordenados if dest in destinos_siglas_da_viagem]
                st.info("ℹ️ Rota de São Paulo detectada. A ordem foi ajustada manualmente.")
            else:
                destinos_ordenados = df_detalhe.sort_values(by='EMIS_MANIF')['DEST_MANIF'].unique()

            # Este loop agora funciona para siglas ou nomes completos
            for dest in destinos_ordenados:
                if dest in locais:
                    ...
                    popup_html = f"""
                        <strong>Destino: {dest}</strong>  

                        Veículo: {placa_selecionada}  

                        Data: {data_selecionada}
                    """
                    folium.Marker(locais[dest], 
                                popup=popup_html,
                                tooltip=dest, 
        ...
        st_folium(mapa_detalhe, use_container_width=True, height=500, key="mapa_detalhe")
